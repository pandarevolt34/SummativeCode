...
initializing class; variable instances:
    ...
    card: stores the card data in the deck
    index: stores the index of the node
...
class RedBlackTree:
    ...
    def insert_card(self, card):  # CORRECT
        ...
        current_node = self.root  # similar to binary search: to be compared with new node
        parent_node = None
    ...
    def find_largest_node(self):  # CORRECT
        ...
        while node != self.nil:
            largest_node = node
            node = node.right  # keep going down the path on the right
        return largest_node
